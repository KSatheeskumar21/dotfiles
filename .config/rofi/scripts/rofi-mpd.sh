...
play(){
    ...
    music="$(mpc listall | rofi -dmenu -p "Music to play")" || exit 1
    mpc -q add "$music"
...
queuePlaylist(){
    ...
    playlist="$(mpc lsplaylists | rofi -dmenu -p "Your playslists")" || exit 1
    mpc -q clear
    mpc load "$playlist"
    mpc -q play 1
...
currentPlaylist(){
    ...
    music="$(echo "${music_list}" | rofi -dmenu -p "Music to play")" || exit 1
    mpc -q clear
